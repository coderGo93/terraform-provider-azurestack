...
import (
	...
	"github.com/hashicorp/terraform-provider-azurestack/internal/services/subscription/validate"
...
var SubscriptionResourceName = "azurestack_subscription"
...
func resourceSubscriptionCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if props := existing.Properties; props != nil {
		return tf.ImportAsExistsError("azurestack_subscription", id.ID())
	}
	...
	if subscriptionIdRaw, ok := d.GetOk("subscription_id"); ok {
		...
		if exists != nil {
			if aliasCount > 1 {
				return fmt.Errorf("multiple Aliases for Subscription %q already exist - to be managed via Terraform only one Alias can exist and this resource needs to be imported into the State. Please see the resource documentation for %q for more information", subscriptionId, "azurestack_subscription")
			}
			return fmt.Errorf("an Alias for Subscription %q already exists with name %q - to be managed via Terraform this resource needs to be imported into the State. Please see the resource documentation for %q for more information", subscriptionId, *exists, "azurestack_subscription")
		}
