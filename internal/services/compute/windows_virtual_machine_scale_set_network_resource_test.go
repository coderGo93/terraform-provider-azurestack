...
locals {
  backend_address_pool_name      = "${azurestack_virtual_network.test.name}-beap"
  frontend_port_name             = "${azurestack_virtual_network.test.name}-feport"
  frontend_ip_configuration_name = "${azurestack_virtual_network.test.name}-feip"
  http_setting_name              = "${azurestack_virtual_network.test.name}-be-htst"
  listener_name                  = "${azurestack_virtual_network.test.name}-httplstn"
  request_routing_rule_name      = "${azurestack_virtual_network.test.name}-rqrt"
}
...
resource "azurestack_subnet" "other" {
  name                 = "other"
  resource_group_name  = "${azurestack_resource_group.test.name}"
  virtual_network_name = "${azurestack_virtual_network.test.name}"
...
resource "azurestack_windows_virtual_machine_scale_set" "test" {
  ...
  network_interface {
    ...
    ip_configuration {
      name                                         = "internal"
      primary                                      = true
      subnet_id                                    = azurestack_subnet.other.id
...
resource "azurestack_application_security_group" "other" {
  name                = "acctestasg2-%d"
  location            = azurestack_resource_group.test.location
  resource_group_name = azurestack_resource_group.test.name
}
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      application_security_group_ids = [
        azurestack_application_security_group.test.id,
        azurestack_application_security_group.other.id,
      ]
    }
  }
}
...
resource "azurestack_public_ip_prefix" "test" {
...
resource "azurestack_windows_virtual_machine_scale_set" "test" {
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      public_ip_address {
        name                = "first"
        public_ip_prefix_id = azurestack_public_ip_prefix.test.id
      }
    }
  }
}
